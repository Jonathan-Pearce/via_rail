"""
Linear Regression Model for Predicting diffMin

This script trains a linear regression model to predict diffMin (delay in minutes)
...
def train_with_cross_validation(X, y, cv_folds=5, random_state=42):
    """
    ...
    Returns:
    --------
    pipeline : trained pipeline (scaler + model)
    cv_results : dict
        Cross-validation results
    """
    print("\n" + "="*60)
    print("TRAINING LINEAR REGRESSION MODEL WITH CROSS-VALIDATION")
    print("="*60)
...
def evaluate_model(pipeline, X, y):
    ...
    pipeline : trained sklearn pipeline
        Trained pipeline with scaler and model
    X : pd.DataFrame
        Feature matrix
    y : pd.Series
        True target values
...
def main():
    ...
    pipeline, cv_results = train_with_cross_validation(
        X, 
        y, 
        cv_folds=5,
        random_state=42
